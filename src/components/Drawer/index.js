/**
 * Ref: https://github.com/Tinysymphony/react-native-drawer-menu
 */
...
export default class Drawer extends Component {
    ...
    _onStartShouldSetPanResponder(evt, gestureState) {
      ...
      const singleDisabled = (this.isLeftOpen && leftDisabled) || (this.isRightOpen && rightDisabled);
    ...
    _handlePanResponderEnd(evt, gestureState) {
      const currentWidth = Math.abs(this._getCurrentDrawerWidth());
      const isOpen = this.isLeftOpen || this.isRightOpen;
    ...
    closeDrawer() {
      ...
      const left = this._getCurrentDrawerWidth();
      new Animation({
        start: left,
        end: 0,
        duration,
    ...
    render() {
      ...
      const drawerContent = this.props.drawerContent ? this._bindDrawerRef(this.props.drawerContent) : null;
      const leftDrawerContent = this.props.leftDrawerContent ? this._bindDrawerRef(this.props.leftDrawerContent) : null;
      const rightDrawerContent = this.props.rightDrawerContent ? this._bindDrawerRef(this.props.rightDrawerContent) : null;
      return (
        <View style={styles.container}>
          ...
          >
            {this.props.children}
            {this.state.showMask && <AnimatableView
              animation="fadeIn"
              easing="ease-in-quad"
              duration={200}
          ...
          {this.isRight &&
            ...
            >
              {rightDrawerContent || drawerContent}
            </View>
                }
        </View>
      );
